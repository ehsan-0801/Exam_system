...
export default function Component(url) {
  ...
  const examId = parseInt(url.params.id);
  ...
  const nextPage = () => {
    ...
    router.push("./CorrectAnsSheet1", { scroll: false });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:3000/api/examprofile/${examId}`,
        {
          method: "PUT",
          headers: {
      ...
      toast.error("An error occurred which is:  ", error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (Data) {
        ...
        setWrongAns(Data.Wrong_ans);
        setBubbleThreshold(Data.bubbleThreshold);
        setStatus(Data.status);
      }
      setExamData(Data);
      // console.log("Exam Profile: ", Data);
    };
        ...
        <form onSubmit={handleSubmit} method="POST">
          <div className="grid grid-cols-5 gap-3 mt-6">
            ...
            <div className="relative my-6">
              ...
              >
                <option value="50 mcq">50 MCQs</option>
                <option value="100 mcq">100 MCQs</option>
              </select>
            </div>
            <div className="relative my-6">
              <label
